...
void get_account_holder(char list_of_account_holders[][MAX_ACCOUNT_HOLDER_SIZE], int account_number) {
  ...
  scanf ("%s", list_of_account_holders[account_number+ARRAY_OFFSET]);
}
...
void output_list_of_account_balances(
  ...
  if (file_pointer == NULL) {
    ...
    while (balances_display_counter <= number_of_accounts) {
      fprintf (file_pointer, "%10s: $%8.2f $%8.2f\n",
        list_of_account_holders[balances_display_counter+ARRAY_OFFSET],
        account_balances[balances_display_counter+ARRAY_OFFSET][START_ACCOUNT_BALANCE],
        account_balances[balances_display_counter+ARRAY_OFFSET][END_ACCOUNT_BALANCE]);
